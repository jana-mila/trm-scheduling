...
def custom_collate_fn(batch, max_seq_len):
    input_feature_dim = batch[0]['inputs'].shape[-1]
    output_feature_dim = batch[0]['labels'].shape[-1]
    padded_problems = torch.zeros(len(batch), max_seq_len, input_feature_dim, dtype=torch.float32)
    
    # Label dimension is fixed at 1
    padded_solutions = torch.full((len(batch), max_seq_len, output_feature_dim), -1.0, dtype=torch.float32)
